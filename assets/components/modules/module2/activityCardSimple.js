 ...
 * @param {boolean} showNotes - Whether to show notes section (default: true)
 * @returns {HTMLElement} - The activity card element
 */
function createActivityCard(title, activities, showNotes = true) {
  ...
  cardHeader.textContent = 'Alege UNA dintre activitățile de mai jos:';
  ...
  activities.forEach((activity, index) => {
    ...
    // Create notes area if showNotes is true
    if (showNotes) {
      ...
      notesLabel.textContent = 'Notele tale (nu salvăm observațiile, sunt doar pentru tine)';

      const notesArea = document.createElement('textarea');
      notesArea.className = 'form-control';
      notesArea.id = `notes-${activity.id}`;
      notesArea.rows = 3;
      notesArea.placeholder = 'Scrie observațiile tale aici...';
 ...
 * @param {boolean} showNotes - Whether to show notes section (default: true)
 */
function initActivityCard(elementId, title, activities, showNotes = false) {
  const targetElement = document.getElementById(elementId);
  if (targetElement) {
    const activityCardModule = createActivityCard(title, activities, showNotes);
